...
- **Supervisor (`npm start`)** — sobe a IA **e** o bot juntos, e reinicia cada um sozinho se cair (recomendado para deixar rodando).
...
O supervisor checa se o `opencode serve` responde em `http://127.0.0.1:4096/global/health`; se não, sobe ele como processo filho, espera ficar saudável e então inicia o bot. Se qualquer um dos dois cair, ele religa sozinho (com backoff). Use `Ctrl+C` para encerrar os dois.
